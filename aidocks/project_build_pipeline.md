...
- **compile:** mirror version into `version.nvgt`, run `nvgt -c -Q cst.nvgt` from `src/` (bundle lands in `src/cst`), copy `data`,`docks`,`sounds` from `cst/` into the bundle, move it to `releases/windows/ClassicStand_windows_portable_password_is_<pw>/cst`.
