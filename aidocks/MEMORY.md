...
- [Game vision](project_game_vision.md) — audio lemonade-stand business sim: serve customers, manage a recipe, reputation (0–200, game over at 0), and money across timed days; the load-bearing systems.
- [Data-driven config](project_data_driven_config.md) — cst/data/config/ (events/stores/tables) is plain-text modder-tunable; parsers in main/parsers/ load it; docks/readme.txt is the format reference.
- [Path conventions](project_path_conventions.md) — src/ (code) + cst/ (assets+launcher) + build/ + releases/ split; the cwd=cst/ trick; no #pragma asset (tools.py copies assets).
...
- [Plan: salt & ice upside](project_plan_salt_ice_upside.md) — DONE in 3.6 (Direction 2, weather-driven): ice=hot-day refreshment, salt=flavor enhancer, +15 tip in the good-drink path, weather-shifted too-icy line, all config-tunable in main.table [thresholds].
- [Plan: rewarding talk](project_plan_rewarding_talk.md) — DONE in 3.6 (Direction B): chatting builds rapport→tip (chatters cap +15, mild +10) and extends chatter patience (+5s/chat, cap +20s); config in customers.table [talk]. Also shipped the patience-pauses-during-serve-menu fix. Calm-a-prickly sliver ships with the mean feature.
- [Plan: mean customer consequences](project_plan_mean_customer_consequences.md) — agreed-but-unimplemented Direction B (social ripple, fully tunable): remove the pay-anyway exemption, bigger rep hit, badmouth ripple saps nearby waiting customers' patience; talk-calming (shared with rewarding-talk) is the counterplay.
- [Plan: per-group pricing](project_plan_per_group_pricing.md) — agreed-but-unimplemented "Pricing station" map menu (new station at x8/y25) with Customer prices (per-type % of base, Direction A) + Sale price; removes the forced daily price popup (base carries over clamped); refuse/patience become per-group; config = defaults/guardrails, menu = player's saved live prices.
- [Plan: storage upgrades](project_plan_storage_upgrades.md) — agreed-but-unimplemented Direction A (per-ingredient preservation upgrades: icebox/pantry/sealed jars/covered jugs, tiered) plus softening the base overnight spoilage rates (esp. ice's brutal 50–100%); rates move to config, upgrades multiply the loss fraction down, bought as one-time owned tiers.
...
- [Don't run or build the game](feedback_dont_run_or_build_the_game.md) — never launch/compile (cst.py, tools.py, nvgt -c); edit and report, the dev runs and verifies.
