...
- [Plan: salt & ice upside](project_plan_salt_ice_upside.md) — agreed-but-unimplemented Direction 2 (weather-driven) design giving salt/ice a real benefit: ice=hot-day refreshment, salt=flavor enhancer, bonus plugs into the good-drink tip/rep path, all config-tunable.
- [Plan: rewarding talk](project_plan_rewarding_talk.md) — agreed-but-unimplemented Direction B (type-specific) design: chatting builds rapport→tip/rep, chatters (elderly/nice/returning) reward it, prickly (mean/critic) risk it with a small calm chance; calm mechanic shared with the mean-customer item.
