...
- [Plan: per-group pricing](project_plan_per_group_pricing.md) — DONE in 3.6: "Pricing station" map menu at x8/y25 with Customer prices (per-type % of base) + Sale price; daily price popup removed (base carries over clamped via clamp_base_price at day/game start); per-group refuse/patience (price_multiplier*group_mult); runtime group_price_mult dict seeded from config [prices], saved per game.
