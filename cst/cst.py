...
# Launch ClassicStand. The .nvgt source lives in src/, but the assets (data, docks, sounds)
# live here in cst/, so we run with cwd set to this folder: every cwd-relative path in the game
# (data/..., docks/..., sounds/...) then resolves against cst/, while the script's own
# #include"includes/..." resolves against src/. We run the console build nvgt.exe (not the
# windowless nvgtw.exe) with CREATE_NO_WINDOW so no console window appears, yet NVGT's compile
# output still reaches a log we can capture -- letting us surface compile errors to errors.txt
# instead of NVGT's own popup. The game opens its own NVGT window.

HERE = os.path.dirname(os.path.abspath(__file__))
SCRIPT = os.path.normpath(os.path.join(HERE, "..", "src", "cst.nvgt"))
...
COMPILE_WAIT = 5
# Where a compile-error report is written: the cst/ folder next to this launcher, so it's always in the
...
def fail(message):
    ctypes.windll.user32.MessageBoxW(0, message, "ClassicStand launcher", 0x10)
    sys.exit(1)
...
if not os.path.isfile(SCRIPT):
    fail("Could not find the ClassicStand script at:\n" + SCRIPT)
...
log_fd, log_path = tempfile.mkstemp(prefix="classicstand_", suffix=".log")
...
try:
    ...
    try:
        ...
        pass
    fail("Failed to start ClassicStand:\n" + str(error))
...
# surface the captured output to errors.txt (in the cst/ folder) and tell the user.
log_file.close()
if code != 0:
    try:
        ...
        details = ""
    body = details if details else "(ClassicStand exited with code %d but printed no output)" % code
    try:
    ...
    try:
        ...
        pass
    fail("ClassicStand failed to start (exit code %d).\nDetails written to:\n%s" % (code, ERRORS_PATH))
